...
public class Dictionary {
    ...
    private volatile Map<Character, CharNode> parentDict;
     ...
     * 词典的目录
     */
    private Dictionary(String appId, Dictionary parentDic) {
        this.appId = appId;
        this.parentDic = parentDic;
        this.parentDict = parentDic.dict;
        this.unit = parentDic.unit;
        initByAppId();
    }

    private void initByAppId() {
     ...
     *            词典的目录
     */
    private static Dictionary getInstance(String appId, File path) {
        File normalizeDir = normalizeFile(path);
        Dictionary dic = null;
        dic = dics.get(normalizeDir);
        if (dic == null) {
            dic = new Dictionary(normalizeDir);
            dics.put(normalizeDir, dic);
        }
        if (appId != null) {
            dic = new Dictionary(appId, dic);
        }
        return dic;
    }
    ...
    private Map<Character, CharNode> loadDicFromDB() throws IOException {
        final Map<Character, CharNode> dic = new HashMap<Character, CharNode>();
        if (appId != null) {
            this.loadWordFromDB(appId, dic);
        }
        return dic;
    }
    ...
    private Map<Character, CharNode> loadDic(File wordsPath) throws IOException {
        ...
        if (charsFile.exists()) {
            ...
            charsFile = new File(this.getClass().getResource("/data/chars.dic").getFile()); // only
                                                                                            // for
                                                                                            // log
        }
        final Map<Character, CharNode> dic = new HashMap<Character, CharNode>();
        ...
        log.info("[Dict Loading] chars loaded time=" + (now() - s) + "ms, line=" + lineNum + ", on file="
                + charsFile.getName());
    ...
    private void loadWord(InputStream is, Map<Character, CharNode> dic, File wordsFile) throws IOException {
        ...
        log.info("[Dict Loading] words loaded time=" + (now() - s) + "ms, line=" + lineNum + ", on file="
                + wordsFile.getName());
    }


    private void loadWordFromDB(String appId, Map<Character, CharNode> dic) throws IOException {
        long s = now();
        int lineNum = load(AppWordDBLoader.getInstance().getWordStreamFromDB(appId), new WordsFileLoading(dic)); // 正常的词库
        log.info("[Dict Loading From DB] words loaded time=" + (now() - s) + "ms, line=" + lineNum + ", for appId="
                + appId);
    }
    ...
    public int maxMatch(char[] sen, int offset) {
        CharNode node = getCharNode(sen[offset]);
    ...
    public Map<Character, CharNode> getDict() {
        ...
        if (this.parentDict != null) {
            map.putAll(this.parentDict);
        }
        if (this.dict != null)
            map.putAll(this.dict);
        return map;
    }
